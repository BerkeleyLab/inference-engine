...
contains
  ...
  module procedure infer
    ...
    do concurrent(n = 1:neurons_per_layer)
      neuron(n,input_layer) = step%activation(dot_product(input_weights(:,n), input(:)) + biases(n,input_layer))
    end do
    do layer = 2, num_layers
      do concurrent(n = 1:neurons_per_layer)
        neuron(n,layer) = step%activation(dot_product(hidden_weights(:,n,layer-1), neuron(:,layer-1)) + biases(n,layer))
      end do
    end do
    allocate(output(num_outputs))
    do concurrent(m = 1:num_outputs)
      output(m) = step%activation(dot_product(output_weights(m,:), neuron(:,num_layers)) + output_biases(m))
    end do
