...
contains
  elemental function y(x_tensor) result(a_tensor)
    ...
    associate(x => x_tensor%values())
      call assert(size(x)==4,"y(x) :: sufficient input")
      a_tensor = tensor_t([(x(1)+x(2))/(x(3)+x(4))])
    end associate
  end function

end module

program learn_avg_speed
  ...
  block
    ...
    associate(num_inputs => trainable_engine%num_inputs(), num_outputs => trainable_engine%num_outputs())
      ...
      block
        ...
        real :: harvest(num_inputs,1000)
        call random_number(harvest)
        inputs = [(tensor_t(harvest(:,i)), i=1,size(harvest,2))]
