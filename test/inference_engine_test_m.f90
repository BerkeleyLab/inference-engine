  ...
  function results() result(test_results)
    ...
    test_results = test_result_t( &
      [ character(len=len("mapping (false,false) to false using the concurrent_dot_products_t() inference strategy")) :: &
        ...
        "training an xor ntwork to map (true,true) to false", &
        "training an xor ntwork to map (true,false) to true", &
        "training an xor ntwork to map (false,true) to true", &
        "training an xor ntwork to map (false,false) to false" &
      ], &
      [ convert_to_and_from_json(), xor_truth_table(concurrent_dot_products_t()), xor_truth_table(matmul_t()), &
        elemental_inference(), single_layer_inference(), train_xor() &
       ] &
    )
  end function
